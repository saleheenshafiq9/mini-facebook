...
const Feed = () => {
  ...
  const addStatusHandler = async (event) => {
    ...
    var postDate =
      formPostDate.getHours() +
      ":" +
      formPostDate.getMinutes() +
      " " +
      formPostDate.toLocaleString("default", { month: "long" }) +
      " " +
      formPostDate.getDate();
    try {
      await sendRequest(
        "http://localhost:81/thoughts",
        "POST",
        JSON.stringify({
